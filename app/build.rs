...
fn compile_shaders() {
    ...
    let output_path = shader_dir_path.parent().unwrap().join("out").join("shaders");

    fs::read_dir(shader_dir_path.clone())
        .unwrap()
        .map(Result::unwrap)
        .filter(|dir| dir.file_type().unwrap().is_file())
        .filter(|dir| dir.path().extension() == Some(OsStr::new("glsl")))
        .filter(|dir| !dir.path().file_name().unwrap().to_str().unwrap().ends_with(".inc.glsl"))
        .for_each(|dir| {
            let path = dir.path();
            let name = path.file_name().unwrap().to_str().unwrap();
            let output_name = output_path.join(format!("{}.spv", &name));
